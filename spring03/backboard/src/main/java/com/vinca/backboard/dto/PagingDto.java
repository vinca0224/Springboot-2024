...
public class PagingDto {
    private int pageSize;           // 화면당 보여지는 게시글의 최대 갯수
    ...
    // 전체 리스트 크기와, 현재 페이지와, 페이지마다 나타낼 글 갯수, 블럭수를 가지고
    // 필요 변수들의 값을 계산하는 생성자
    public PagingDto(Long totalListSize, Integer page, Integer pageSize, Integer blockSize){
        ...
        // 블럭 마지막 페이지 검증(한 블럭이 10페이지가 안넘으면 마지막 페이지를 최대 페이지 수로 다시 변경 10 -> 3)
        ...
        // 이전 블럭 검증
        if (this.prevBlock < 1) {   // 1페이지보다 작을 수 없음
            this.prevBlock = 1;
        }
        // 다음블럭
        ...
        if (this.nextBlock > this.totalPageNum) {
            this.nextBlock = this.totalPageNum;
        }
        // 시작 인덱스 번호
